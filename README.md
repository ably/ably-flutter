...
1. Get or create a key, typically in one of these ways:
    ...
    - creating a random one on one client using: `ably.Crypto.generateRandomKey();`. You must to distribute this specific key to other clients, since calling `generateRandomKey` on another device will generate a different key.
