...
class Codec extends StandardMessageCodec {
  ...
  ClientOptions decodeClientOptions(Map<String, dynamic> jsonMap) {
    ...
    v.tokenDetails = decodeTokenDetails(
        toJsonMap(readFromJson<Map>(jsonMap, TxClientOptions.tokenDetails)));
    v.authHeaders =
        readFromJson<Map<String, String>>(jsonMap, TxClientOptions.authHeaders);
    v.authParams =
    ...
    v.defaultTokenParams = decodeTokenParams(toJsonMap(
    ...
    v.transportParams = readFromJson<Map<String, String>>(
  ...
  TokenDetails decodeTokenDetails(Map<String, dynamic> jsonMap) {
    if (jsonMap == null) return null;
    TokenDetails v =
        TokenDetails(readFromJson<String>(jsonMap, TxTokenDetails.token));
  ...
  TokenParams decodeTokenParams(Map<String, dynamic> jsonMap) {
    ...
    v.timestamp = DateTime.fromMillisecondsSinceEpoch(
  ...
  ConnectionEvent decodeConnectionEvent(int index) =>
      (index == null) ? null : ConnectionEvent.values[index];

  /// Decodes [index] value to [ConnectionState] enum if not null
  ConnectionState decodeConnectionState(int index) =>
      (index == null) ? null : ConnectionState.values[index];

  /// Decodes [index] value to [ChannelEvent] enum if not null
  ChannelEvent decodeChannelEvent(int index) =>
      (index == null) ? null : ChannelEvent.values[index];

  /// Decodes [index] value to [ChannelState] enum if not null
  ChannelState decodeChannelState(int index) =>
      (index == null) ? null : ChannelState.values[index];
  ...
  ConnectionStateChange decodeConnectionStateChange(
      Map<String, dynamic> jsonMap) {
    if (jsonMap == null) return null;
    ConnectionState current = decodeConnectionState(
        readFromJson<int>(jsonMap, TxConnectionStateChange.current));
    ConnectionState previous = decodeConnectionState(
        readFromJson<int>(jsonMap, TxConnectionStateChange.previous));
    ConnectionEvent event = decodeConnectionEvent(
        readFromJson<int>(jsonMap, TxConnectionStateChange.event));
  ...
  ChannelStateChange decodeChannelStateChange(Map<String, dynamic> jsonMap) {
    if (jsonMap == null) return null;
    ChannelState current = decodeChannelState(
        readFromJson<int>(jsonMap, TxChannelStateChange.current));
    ChannelState previous = decodeChannelState(
        readFromJson<int>(jsonMap, TxChannelStateChange.previous));
    ChannelEvent event = decodeChannelEvent(
        readFromJson<int>(jsonMap, TxChannelStateChange.event));
  ...
  Message decodeChannelMessage(Map<String, dynamic> jsonMap) {
    ...
    message.connectionId =
        readFromJson<String>(jsonMap, TxMessage.connectionId);
