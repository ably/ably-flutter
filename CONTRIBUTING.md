...
- **Resource consumption tradeoffs:** Firebase launches an isolate capable of only handling messages at app launch, even if users' application isn't handling remote messages. Firebase Messaging keeps this isolate running throughout the app, and have a queue process to queue messages by maintaining an Android Service. This allows 10 minutes of execution time, where as on iOS, Firebase Messaging only has 30 seconds of execution time. Instead, Ably Flutter launch a new isolate on every message if the application is not yet running. Instead of creating a service and queueing work for it for each message. FCM will hold the messages until the application is ready to process the messages. A new message will spawn a new engine.
...
Because of this architectural simplicity, Ably Flutter do not need to use [`PluginUtilies`](https://stackoverflow.com/questions/69208164/what-does-pluginutilities-do-in-flutters-dartui-library-and-how-do-i-use/69208165#69208165), pass references of 2 methods between Dart and platform side, or save and load these methods in `SharedPreferences`. Ably Flutter avoid conflicts between the default `FlutterEngine` launched in an `Activity` and by ourselves, by using separate method channels (unique channel names).
