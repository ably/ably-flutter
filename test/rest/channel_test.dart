...
void main() {
  ...
  // Created channel instances
  final channels = <int, AblyMessage>{};

  // Used to pass the authCallback to the methodCallHandler because the
  // BinaryMessenger drops function references.
  // `authUrl` is misused to signal an auth callback because the function
  // reference seems to be just dropped by the BinaryMessenger used by
      ...
      switch (methodCall.method) {
        ...
        case PlatformMethod.publish:
          ...
          if (!isAuthenticated && clientOptions.authUrl == 'hasAuthCallback') {
            await authCallback(clientOptions.defaultTokenParams);
            AblyMethodCallHandler(methodChannel).onAuthCallback(AblyMessage(
  ...
  test('publish message with authCallback', () async {
    // setup
    authCallback = expectAsync1((token) async {}, max: 2);
  ...
  test('publish message with authCallback timing out', () async {
    ...
    unawaited(
      fakeAsync((async) async {
        authCallback = (token) async => Future.delayed(tooMuchDelay);
        ...
        // verification
        expect(future1, throwsA(isA<TimeoutException>()));
        expect(future2, throwsA(isA<TimeoutException>()));
