...
final class StreamsChannel {
  ...
  private final class IncomingStreamRequestHandler implements BinaryMessageHandler {
    ...
    private final ConcurrentHashMap<Integer, StreamsChannel.Stream> streams =
    ...
    private void onListen(int id, Object arguments, BinaryReply callback) {
      final Stream stream =
          new Stream(
              new IncomingStreamRequestHandler.EventSinkImplementation(id),
    ...
    void clearAll() {
      for (ConcurrentHashMap.Entry<Integer, StreamsChannel.Stream> entry :
          incomingStreamRequestHandler.streams.entrySet()) {
        int id = entry.getKey();
