 ...
 * BackgroundIsolateAndroidPlatform to avoid exceptions being thrown in the case where an Isolate
 * doesn't need to be manually launched (App is already running), so
 * `pushBackgroundFlutterApplicationReadyOnAndroid` platform method call doesn't need to be called.
 */
public class BackgroundMethodCallHandler implements MethodChannel.MethodCallHandler {
  ...
  public void onMethodCall(@NonNull MethodCall call, @NonNull MethodChannel.Result result) {
    if (call.method.equals(pushBackgroundFlutterApplicationReadyOnAndroid)) {
      Log.v(
          TAG,
          "Ignoring pushBackgroundFlutterApplicationReadyOnAndroid because it doesn't need to be"
              + " called, since the user's Flutter app (default DartEntrypoint) is already"
              + " running.");
    } else {
