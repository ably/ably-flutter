...
public class AblyFlutter implements FlutterPlugin, ActivityAware, PluginRegistry.NewIntentListener {
  ...
  private void setupChannels(BinaryMessenger messenger, Context applicationContext) {
    ...
    final StreamsChannel streamsChannel =
        new StreamsChannel(messenger, "io.ably.flutter.stream", codec);
    ...
    methodCallHandler =
        new AblyMethodCallHandler(methodChannel, streamsChannel, applicationContext);
    ...
    handleRemoteMessageIntent(
        ...
        (message) -> {
          ...
          methodChannel.invokeMethod(
              PlatformConstants.PlatformMethod.pushOnNotificationTap, message);
        });
    return false;
  }
