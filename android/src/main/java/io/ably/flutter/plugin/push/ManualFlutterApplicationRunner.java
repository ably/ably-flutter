 ...
 * [PushMessagingEventHandlers.java] to see where push notifications being handled whilst the app is
 * in the background or the foreground. Use this class when no existing Flutter Activity is running.
 *
 * <p>This class can be generalized to launch the app manually for any purpose, but currently it is
...
public class ManualFlutterApplicationRunner implements MethodChannel.MethodCallHandler {
   ...
   */
  public ManualFlutterApplicationRunner(
    ...
    backgroundMethodChannel =
        new MethodChannel(
            ...
            new StandardMethodCodec(new AblyMessageCodec(new CipherParamsStorage())));
          ...
          new MethodChannel.Result() {
            ...
            public void error(
              ...
              Log.e(
                  TAG,
                  String.format(
                      "pushOnBackgroundMessage method call from Java to Dart returned error.\n "
                          + "errorCode: %s\n"
                          + "errorMessage: %s\n"
                          + "errorDetails: %s\n",
            ...
            public void notImplemented() {
              Log.e(
                  TAG,
                  "Method: pushOnBackgroundMessage did not receive message on dart side. Either the"
                      + " binding has not been initialized or the method call handler was not"
                      + " registered to receive this method.");
              finish();
            }
          });
    } else {
